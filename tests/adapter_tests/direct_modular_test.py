        ...
        for test_case in TEST_PROMPTS:
            ...
            try:
                ...
                if result and "text" in result:
                    ...
                    # Extract code
                    code = code_formatter.extract_code_from_response(generated_text, test_case['language'])
                    
                    if code and len(code.strip()) > 0:
                        logger.info(f"Successfully extracted code!")
                        
                        # Save the code to a file
                        output_path = os.path.join(os.path.dirname(__file__), "test_output", test_case['output_file'])
                ...
                duration = time.time() - start_time
                logger.exception(f"Exception during generation after {duration:.2f}s: {str(e)}")

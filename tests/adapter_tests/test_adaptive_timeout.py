...
async def test_adaptive_timeout():
    ...
    try:
        ...
        from gollm.llm.providers.ollama.modules.prompt.code_formatter import CodePromptFormatter
        import aiohttp
            ...
            for test_case in TEST_PROMPTS:
                ...
                try:
                    ...
                    if success:
                        ...
                        clean_code = code_formatter.extract_code_from_response(generated_text, test_case['language'])
                        
                        # Save the generated code
                        output_path = os.path.join(os.path.dirname(__file__), test_case['output_file'])
                        with open(output_path, 'w') as f:
                            f.write(clean_code)
                        
                        logger.info(f"Saved generated code to {test_case['output_file']}")
                        
                        # Validate the code quality
                        quality_score = validate_code_quality(clean_code, test_case['language'])
...
def simulate_tests() -> List[Dict[str, Any]]:
    ...
    for test_case in TEST_PROMPTS:
        ...
        results.append(test_result)
        logger.info(f"Simulated test for {test_case['name']}: {'✅ Success' if success else '❌ Failed'} in {duration:.2f}s")
...
async def generate_report(results: List[Dict[str, Any]]):
    ...
    with open(report_path, 'w') as f:
        ...
        f.write("| Test | Prompt Length | Expected Timeout | Actual Duration | Status | Quality |\n")
        f.write("|------|--------------|-----------------|-----------------|--------|---------|\n")
        ...
        for result in results:
            ...
            f.write(f"| {name} | {prompt_length} | {expected_timeout}s | {duration:.2f}s | {success} | {quality} |\n")
        ...
        f.write("\n## Notes\n\n")
        f.write("- Timeout values are calculated based on prompt length and complexity\n")
        f.write("- Code quality is scored on a scale of 0-10 based on various factors\n")
...
async def main():
    ...
    try:
        ...
        generated_files = [r.get("output_file") for r in results if r.get("success", False) and "output_file" in r]
        if generated_files:

...
async def test_chat_code_generation():
    ...
    async with OllamaModularAdapter(CONFIG) as adapter:
        ...
        if "error" not in result or not result["error"]:
            ...
            logger.error(f"Chat generation failed: {result.get('error', 'Unknown error')}")
            if "details" in result:
                logger.error(f"Details: {result['details']}")
...
async def main():
    """Run all tests."""
    logger.info("Starting direct test of Ollama modular adapter with improved code generation")
    ...
    try:
        ...
        for i, (prompt, result, duration) in enumerate(results):
            ...
            logger.info(f"Prompt {i+1}: {'Success' if success else 'Failed'} in {duration:.2f}s")
        
        chat_success = "error" not in chat_result or not chat_result["error"]
        logger.info(f"Chat test: {'Success' if chat_success else 'Failed'} in {chat_duration:.2f}s")

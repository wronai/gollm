...
def calculate_adaptive_timeout(prompt_length: int, is_code_generation: bool = False, language: str = None) -> int:
    """
...
def direct_api_call(prompt: str, language: str = None) -> Dict[str, Any]:
    ...
    timeout = calculate_adaptive_timeout(prompt_length, is_code_generation=True, language=language)
    ...
    curl_cmd = [
        'curl', '-s', '-X', 'POST', 'http://rock:8081/api/chat',
...
def main():
    ...
    for result in results:
        status = "u2705 Success" if result.get("success", False) and result.get("code_extracted", False) else "u274c Failed"
        logger.info(f"{result['name']}: {status} (Timeout: {result['timeout']}s, Duration: {result.get('duration', 'N/A')}s)")

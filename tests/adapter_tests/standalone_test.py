...
def calculate_adaptive_timeout(prompt_length: int, is_code_generation: bool = False) -> float:
    """
    ...
    # For medium prompts (100-1000 chars), scale linearly
        ...
        timeout = base_timeout + (prompt_length - 100) * 0.05  # Add 0.05s per char over 100
    # For long prompts (1000+ chars), scale more aggressively
    else:
        timeout = base_timeout + 45.0 + (prompt_length - 1000) * 0.02  # Add 0.02s per char over 1000
...
async def test_adaptive_timeout():
    ...
    for prompt_length, is_code_generation in test_cases:
        ...
        logger.info(f"Prompt length: {prompt_length}, Task type: {task_type}, Timeout: {timeout:.2f}s")

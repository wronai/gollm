    ...
    async with OllamaModularAdapter(CONFIG) as adapter:
        ...
        if result.get("success", False):
            ...
            if "original_text" in result:
                logger.info(f"Response was cleaned up, removing {len(result['original_text']) - len(result['text'])} characters")
...
async def test_code_generation_chat():
    ...
    async with OllamaModularAdapter(CONFIG) as adapter:
        ...
        if result.get("success", False):
            ...
            logger.error(f"Chat code generation failed: {result.get('error', 'Unknown error')}")
...
async def test_auto_detection():
    ...
    async with OllamaModularAdapter(CONFIG) as adapter:
        ...
        if result.get("success", False):
            ...
            logger.error(f"Auto-detected code generation failed: {result.get('error', 'Unknown error')}")
...
"""
        ...
        if result.get("success", False):
            ...
            logger.error(f"Code generation with context failed: {result.get('error', 'Unknown error')}")

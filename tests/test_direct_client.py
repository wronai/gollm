...
async def test_generate(use_grpc=False):
    ...
    async with DirectLLMClient(base_url="http://localhost:11434", use_grpc=use_grpc) as client:
...
async def test_chat(use_grpc=False):
    ...
    async with DirectLLMClient(base_url="http://localhost:11434", use_grpc=use_grpc) as client:
        result = await client.chat_completion(
            model="deepseek-coder:1.3b",
            messages=[
                {"role": "user", "content": "Explain the difference between HTTP and gRPC in 3 sentences."}
            ],
            temperature=0.1,
            max_tokens=500
        )

...
def main():
    ...
    try:
        from gollm.llm.providers.ollama.migration import (
            is_migration_needed, apply_migration, rollback_migration
        )
    except ImportError:
        logger.error("Failed to import migration utilities. Make sure you're running this script from the correct directory.")
        sys.exit(1)

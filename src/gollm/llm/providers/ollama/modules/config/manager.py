...
class ConfigManager:
    ...
    def load_config(self, config_dict: Optional[Dict[str, Any]] = None) -> OllamaConfig:
        ...
        logger.info(f"Loaded Ollama configuration: model={config.model}, base_url={config.base_url}")
        return config
    ...
    def _apply_env_vars(self, config: OllamaConfig) -> None:
        ...
        if 'OLLAMA_TIMEOUT' in os.environ:
            try:
                ...
                logger.warning(f"Invalid OLLAMA_TIMEOUT value: {os.environ['OLLAMA_TIMEOUT']}")
        ...
        if 'OLLAMA_MAX_TOKENS' in os.environ:
            try:
                ...
                logger.warning(f"Invalid OLLAMA_MAX_TOKENS value: {os.environ['OLLAMA_MAX_TOKENS']}")
        ...
        if 'OLLAMA_TEMPERATURE' in os.environ:
            try:
                ...
                logger.warning(f"Invalid OLLAMA_TEMPERATURE value: {os.environ['OLLAMA_TEMPERATURE']}")
    ...
    def _validate_config(self, config: OllamaConfig) -> None:
        ...
        if not 0 <= config.temperature <= 1:
            logger.warning(f"Temperature must be between 0 and 1, got: {config.temperature}")

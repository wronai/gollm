...
try:
    ...
    try:
        from gollm.llm.providers.ollama.grpc import OllamaGrpcAdapter, GRPC_AVAILABLE
    except ImportError:
        GRPC_AVAILABLE = False
except ImportError:
    logger.error("Failed to import Ollama modules. Make sure you're running this script from the correct directory.")
    sys.exit(1)
...
async def benchmark_http(config: OllamaConfig, prompt: str, iterations: int = 3) -> Dict[str, Any]:
...
async def benchmark_grpc(config: OllamaConfig, prompt: str, iterations: int = 3) -> Dict[str, Any]:
...
async def run_benchmarks(config: OllamaConfig, prompt_type: str, iterations: int) -> Dict[str, Any]:
...
async def main():
    ...
    if args.prompt_type == 'all':
        for prompt_type in TEST_PROMPTS.keys():
            results[prompt_type] = await run_benchmarks(config, prompt_type, args.iterations)
    else:
        results[args.prompt_type] = await run_benchmarks(config, args.prompt_type, args.iterations)
    ...
    for prompt_type, result in results.items():
        ...
        print("HTTP Adapter:")
        print(f"  Successful iterations: {http_stats['successful']}/{http_stats['iterations']}")
        if "avg_duration" in http_stats:
            print(f"  Average duration: {http_stats['avg_duration']:.2f}s")
            print(f"  Min/Max duration: {http_stats['min_duration']:.2f}s / {http_stats['max_duration']:.2f}s")
        ...
        if "error" in grpc_stats:
            ...
            print(f"  Successful iterations: {grpc_stats['successful']}/{grpc_stats['iterations']}")
            if "avg_duration" in grpc_stats:
                print(f"  Average duration: {grpc_stats['avg_duration']:.2f}s")
                print(f"  Min/Max duration: {grpc_stats['min_duration']:.2f}s / {grpc_stats['max_duration']:.2f}s")

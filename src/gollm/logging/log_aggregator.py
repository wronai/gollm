...
class LogAggregator:
    ...
    def _get_frequent_failures(self) -> List[str]:
        ...
        return [
            ...
            for cmd, count in sorted(
                command_failures.items(), key=lambda x: x[1], reverse=True
            )
        ][:3]
    ...
    def _analyze_execution_trends(self) -> Dict[str, Any]:
        ...
        success_rate = sum(1 for ex in recent_executions if ex.exit_code == 0) / len(
            recent_executions
        )
        avg_execution_time = sum(ex.execution_time for ex in recent_executions) / len(
            recent_executions
        )
    ...
    def _save_execution_log(self, context: ExecutionContext):
        ...
        log_file = (
            self.logs_dir / f"execution_{datetime.now().strftime('%Y%m%d')}.jsonl"
        )
    ...
    def _load_history(self):
        ...
        for log_file in sorted(self.logs_dir.glob("execution_*.jsonl"))[
            -7:
        ]:  # Ostatnie 7 dni
            try:

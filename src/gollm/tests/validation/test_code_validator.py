...
from gollm.validation.code_validator import (
...
"""
    ...
    def test_prompt_detection(self):
        ...
        prompt = "Create a Python class for a user with name, email, and password fields."
...
```
"""
        is_valid, validated_code, issues = validate_and_extract_code(prompt_with_code, 'py')
        self.assertTrue(is_valid)
...
def hello_world():
    print("Hello, World!
"""
        is_valid, validated_code, issues = validate_and_extract_code(code_with_error, 'py')
        self.assertTrue(is_valid)  # Should be valid because we fixed it
        self.assertNotEqual(validated_code, code_with_error)  # Should be different after fixing
    ...
    def test_real_world_example(self):
        ...
        is_valid, validated_code, issues = validate_and_extract_code(problematic_content, 'py')
        self.assertFalse(is_valid)

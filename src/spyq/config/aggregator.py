...
class ProjectConfigAggregator:
    ...
    def _merge_config(self, filename: str, parsed_config: Dict[str, Any]):
        ...
        if filename == 'spyq.json':
            self.aggregated_config["spyq_rules"] = parsed_config
    ...
    def _detect_conflicts(self):
        ...
        # SPYQ
        spyq_rules = self.aggregated_config["spyq_rules"].get("validation_rules", {})
        if "max_line_length" in spyq_rules:
            line_length_configs["spyq"] = spyq_rules["max_line_length"]
    ...
    def _generate_recommendations(self):
        ...
        # Sprawdź czy SPYQ ma wszystkie potrzebne konfiguracje
        spyq_config = self.aggregated_config["spyq_rules"]
        if not spyq_config:
            recommendations.append({
                "type": "missing_spyq_config",
                "priority": "high",
                "description": "No SPYQ configuration found",
                "action": "Create spyq.json with validation rules"
            })
        ...
        if "flake8" in self.aggregated_config["linting_rules"]:
            recommendations.append({
                ...
                "priority": "medium", 
                "description": "Sync SPYQ rules with existing Flake8 configuration",
                "action": "Update SPYQ rules to match Flake8 standards"
            })
    ...
    def get_llm_config_summary(self) -> str:
        ...
        # SPYQ rules
        spyq_rules = config.get("spyq_rules", {}).get("validation_rules", {})
        if spyq_rules:
            active_rules.append("SPYQ Quality Rules:")
            for rule, value in spyq_rules.items():

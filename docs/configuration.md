...
# SPYQ Configuration Guide

## 📋 Overview

SPYQ uses a `spyq.json` file for configuration. This guide covers all available settings and how to customize them for your project.
...
```bash
# Development
cp spyq.dev.json spyq.json

# Production  
cp spyq.prod.json spyq.json

# Testing
cp spyq.test.json spyq.json
```
...
spyq config set validation_rules.max_function_lines 40
...
spyq config migrate --to-version 0.2.0

# Backup created automatically as spyq.json.backup
```

### Manual Migration
```bash
# Check configuration compatibility
spyq config check-compatibility

# Show migration recommendations
spyq config migration-guide
```
...
# SPYQ LLM Integration Guide

## 🎯 Overview

SPYQ integrates with Large Language Models to provide intelligent code generation, automatic fixes, and context-aware suggestions. This guide covers setup, usage, and best practices.
...
spyq config set llm_integration.providers.openai.model gpt-4

# 3. Test connection
spyq generate "Create a hello world function"
```
...
# 2. Enable in SPYQ
spyq config set llm_integration.enabled true
spyq config set llm_integration.providers.anthropic.enabled true
spyq config set llm_integration.providers.anthropic.model claude-3-sonnet-20240229

# 3. Test connection
spyq generate "Create a data validation class"
```
...
# Complex logic
spyq generate "Create a payment processor that handles multiple payment methods"
```

### Context-Aware Generation
SPYQ automatically provides rich context to the LLM:
...
spyq generate "Create a user registration system"
```
...
# Interactive fixing
spyq fix --interactive
```

### Iterative Improvement
```bash
# SPYQ automatically iterates with LLM until quality standards are met:

$ spyq generate "Create a data processor"
...
spyq config set llm_integration.rate_limiting.requests_per_minute 20
spyq config set llm_integration.rate_limiting.retry_delay_seconds 5
```

#### 3. Context Too Large
```bash
# Reduce context size
spyq config set llm_integration.context_settings.max_context_files 3
spyq config set llm_integration.context_settings.context_window_size 4000
```

#### 4. Poor Code Quality
```bash
# Increase iterations
spyq config set llm_integration.quality_control.max_iterations 5

# Raise quality threshold
spyq config set llm_integration.quality_control.min_quality_score 90
...
spyq config set debug.enabled true
spyq config set debug.save_llm_conversations true

# Check debug logs
tail -f .spyq/logs/llm_debug.log
```
...
spyq generate "Create a REST API endpoint for user registration with email validation, password hashing, and proper error responses"

# Include context
spyq generate "Add payment processing to our existing e-commerce system, following the patterns in src/orders/"

# Specify constraints
spyq generate "Create a data validation function under 30 lines with comprehensive docstring"
```

### 2. Iterative Development
```bash
# Start simple
spyq generate "Create basic user model"

# Add features incrementally  
spyq generate "Add authentication methods to existing User class"
spyq generate "Add password reset functionality to User class"
```

### 3. Quality Assurance
```bash
# Always validate generated code
spyq validate generated_file.py
...
spyq config set llm_integration.providers.openai.model gpt-3.5-turbo

# Monitor usage
spyq stats --llm --period month

# Set usage limits
spyq config set llm_integration.usage_limits.monthly_tokens 100000
```
...
spyq generate "create user auth" --review

# Check for security issues
spyq validate --security-scan generated_code.py
...
# Local privacy
spyq config set llm_integration.providers.ollama.model codellama:13b
```
